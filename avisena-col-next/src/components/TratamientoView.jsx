...
export default function TratamientoView() {
  ...
  useEffect(() => {
    if (location.state && location.state.autofill) {
      setDefaultFormData({
        fechaInicio: location.state.fechaInicio || '',
        galpon: location.state.galpon || '1',
        lote: location.state.lote || 'L1',
        observaciones: location.state.observaciones || '',
      });
      setMostrarFormulario(true);
      // Clear location state so future openings are blank
